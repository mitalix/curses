...
def shoe_horn(stdscr):
    ...
    ## ## ############################################
        ...
        Enter = False # have to put it somewhere
    ...
    ## ## ############################################
        for line, element in enumerate(box1_buffer):
            box1.addstr(line + 3,2, element[:this_x - (border * 2)])
        for line, element in enumerate(box2_buffer):
            box2.addstr(line + 3,2, element[:this_x - (border * 2)])
        for line, element in enumerate(box3_buffer):
            box3.addstr(line + 3,2, element[:this_x - (border * 2)])
    ## ## ############################################
    ...
    ## ## ############################################
        if focus[0] == "box1":
            ...
            if height - heading_offset > len(box1_buffer[box1_index:]):
                ...
                stretch = height - heading_offset
            box1_pointer = (box1_pointer + pointer) % (stretch)# if pointer else False
            box1.addstr(box1_pointer + heading_offset - 1,2,box1_buffer[box1_index+box1_pointer], curses.A_REVERSE | curses.A_ITALIC | curses.A_UNDERLINE | curses.A_BOLD)
    ...
    ## ## ############################################
            ...
            if height - heading_offset > len(box2_buffer[box2_index:]):
                ...
                stretch = height - heading_offset
            box2_pointer = (box2_pointer + pointer) % (stretch)# if pointer else False
            box2.addstr(box2_pointer + heading_offset - 1,2,box2_buffer[box2_index+box2_pointer], curses.A_REVERSE | curses.A_ITALIC | curses.A_UNDERLINE | curses.A_BOLD)
            # box1.addstr(box1_pointer + heading_offset - 1,2,"BINGO", curses.A_REVERSE | curses.A_ITALIC | curses.A_UNDERLINE | curses.A_BOLD)
    ## ## ############################################
        elif focus[0] == "box3":
            ...
            if height - heading_offset > len(box1_buffer[box3_index:]):
                ...
                stretch = height - heading_offset
            box3_pointer = (box3_pointer + pointer) % (stretch)# if pointer else False
            box3.addstr(box3_pointer + heading_offset - 1,2,box3_buffer[box3_index+box3_pointer], curses.A_REVERSE | curses.A_ITALIC | curses.A_UNDERLINE | curses.A_BOLD)
        pointer = 0
    ## ## ############################################
    ## ## Status messages
    ## ## ############################################
